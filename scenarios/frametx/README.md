...
implements the behavior. As of ethrex `eip8141-v2-lenient` the `atomic-fail` shape mismatches
because the frame-entry EIP-2929 access charge is not applied, so a frame budgeted 1 execution gas
succeeds instead of halting. Every frame consequently reports zero execution gas used.

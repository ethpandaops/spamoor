...
func (s *Scenario) Run(ctx context.Context) error {
	...
	s.logger.Infof("deployed nft contract: %v (confirmed in block #%v)", s.contractAddr.String(), deployBlock)
...
func (s *Scenario) sendDeploymentTx(ctx context.Context) (*common.Address, uint64, error) {
	...
	initCodeBytes := common.FromHex(contract.TestToken721MetaData.Bin)
...
func (s *Scenario) sendTx(ctx context.Context, txIdx uint64) (scenario.ReceiptChan, *types.Transaction, *spamoor.Client, *spamoor.Wallet, error) {
	...
	tx, err := wallet.BuildBoundTx(ctx, &txbuilder.TxMetadata{
		...
		// transferMint mints an NFT and transfers it to a (possibly fresh)
		// recipient. Under the Amsterdam fee schedule, creating the owner/balance
		// slots pushes this to ~245k gas; keep static headroom rather than
		// estimating per spam tx.
		Gas:   350000,

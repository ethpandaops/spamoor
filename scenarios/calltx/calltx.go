...
func (s *Scenario) Init(options *scenario.Options) error {
	...
	if blockLimit > 0 && s.options.DeployGasLimit > blockLimit {
		s.logger.Warnf("Deploy gas limit %d exceeds block gas limit %d and will most likely be dropped by the execution layer client", s.options.DeployGasLimit, blockLimit)
	}
	if blockLimit > 0 && s.options.GasLimit > blockLimit {
...
func (s *Scenario) sendDeploymentTx(ctx context.Context, contractCode []byte) (*txtypes.Receipt, *spamoor.Client, error) {
...
func (s *Scenario) sendTx(ctx context.Context, txIdx uint64) (scenario.ReceiptChan, *txtypes.Transaction, *spamoor.Client, *spamoor.Wallet, error) {
	...
	if s.abiCallBuilder != nil {
		...
		if err != nil {
			return nil, nil, client, wallet, fmt.Errorf("failed to build ABI call data: %w", err)
		}

...
type WalletPool struct {
	...
	txpool      spamoortypes.TxPool
	runFundings bool

	childWallets     []spamoortypes.Wallet
	wellKnownNames   []*spamoortypes.WellKnownWalletConfig
	wellKnownWallets map[string]spamoortypes.Wallet
...
func NewWalletPool(ctx context.Context, logger logrus.FieldLogger, rootWallet *RootWallet, clientPool spamoortypes.ClientPool, txpool spamoortypes.TxPool) spamoortypes.WalletPool {
	return &WalletPool{
		ctx:              ctx,
		logger:           logger,
		rootWallet:       rootWallet,
		...
		txpool:           txpool,
		childWallets:     make([]spamoortypes.Wallet, 0),
		wellKnownWallets: make(map[string]spamoortypes.Wallet),
		runFundings:      true,
	}
}
...
func (pool *WalletPool) GetTxPool() spamoortypes.TxPool {
	return pool.txpool
}

// GetClientPool returns the client pool used for blockchain interactions.
func (pool *WalletPool) GetClientPool() spamoortypes.ClientPool {
	return pool.clientPool
}

// GetRootWallet returns the root wallet that funds all child wallets.
func (pool *WalletPool) GetRootWallet() spamoortypes.Wallet {
	return pool.rootWallet.GetWallet()
}

func (pool *WalletPool) WithRootWalletLock(ctx context.Context, txCount int, lockedLogFn func(), lockedFn func() error) error {
	return pool.rootWallet.WithWalletLock(ctx, txCount, lockedLogFn, lockedFn)
}
...
func (pool *WalletPool) AddWellKnownWallet(config *spamoortypes.WellKnownWalletConfig) {
...
func (pool *WalletPool) GetClient(mode spamoortypes.ClientSelectionMode, input int, group string) spamoortypes.Client {
...
func (pool *WalletPool) GetWallet(mode spamoortypes.WalletSelectionMode, input int) spamoortypes.Wallet {
	...
	switch mode {
	case spamoortypes.SelectWalletByIndex:
		input = input % len(pool.childWallets)
	case spamoortypes.SelectWalletRandom:
		input = rand.Intn(len(pool.childWallets))
	case spamoortypes.SelectWalletRoundRobin:
...
func (pool *WalletPool) GetWellKnownWallet(name string) spamoortypes.Wallet {
...
func (pool *WalletPool) GetAllWallets() []spamoortypes.Wallet {
	wallets := make([]spamoortypes.Wallet, len(pool.childWallets)+len(pool.wellKnownWallets))
...
func (pool *WalletPool) PrepareWallets() error {
	...
	if pool.config.WalletCount == 0 && len(pool.wellKnownWallets) == 0 {
		pool.childWallets = make([]spamoortypes.Wallet, 0)
	} else {
		var client spamoortypes.Client
		var fundingReqs []*FundingRequest

		for i := 0; i < 3; i++ {
			client = pool.clientPool.GetClient(spamoortypes.SelectClientRandom, 0, "") // send all preparation transactions via this client to avoid rejections due to nonces
			if client == nil {
				return fmt.Errorf("no client available")
			}

			pool.childWallets = make([]spamoortypes.Wallet, 0, pool.config.WalletCount)
			...
			for _, config := range pool.wellKnownNames {
				wg.Add(1)
				wl <- true
				go func(config *spamoortypes.WellKnownWalletConfig) {
					defer func() {
						<-wl
						wg.Done()
					}()
...
func (pool *WalletPool) prepareChildWallet(childIdx uint64, client spamoortypes.Client, seed string) (spamoortypes.Wallet, *FundingRequest, error) {
...
func (pool *WalletPool) prepareWellKnownWallet(config *spamoortypes.WellKnownWalletConfig, client spamoortypes.Client, seed string) (spamoortypes.Wallet, *FundingRequest, error) {
...
func (pool *WalletPool) prepareWallet(privkey string, client spamoortypes.Client, refillAmount *uint256.Int, refillBalance *uint256.Int) (spamoortypes.Wallet, *FundingRequest, error) {
...
func (pool *WalletPool) resupplyChildWallets() error {
	client := pool.clientPool.GetClient(spamoortypes.SelectClientRandom, 0, "")
	if client == nil {
		return fmt.Errorf("no client available")
	}
	...
	for idx, config := range pool.wellKnownNames {
		...
		wg.Add(1)
		wl <- true
		go func(idx int, childWallet spamoortypes.Wallet, config *spamoortypes.WellKnownWalletConfig) {
			defer func() {
				<-wl
				wg.Done()
			}()
...
func (pool *WalletPool) processFundingRequests(fundingReqs []*FundingRequest) error {
	...
	return pool.rootWallet.WithWalletLock(pool.ctx, batchTxCount, func() {
		...
		for txIdx := 0; txIdx < len(txList); txIdx += 200 {
			...
			err := pool.txpool.SendAndAwaitTxRange(pool.ctx, pool.rootWallet.wallet, txList[txIdx:endIdx], &spamoortypes.SendTransactionOptions{
				Client: client,
...
func (pool *WalletPool) buildWalletFundingTx(childWallet spamoortypes.Wallet, client spamoortypes.Client, refillAmount *uint256.Int) (*types.Transaction, error) {
	if client == nil {
		client = pool.clientPool.GetClient(spamoortypes.SelectClientByIndex, 0, "")
		if client == nil {
...
func (pool *WalletPool) buildWalletFundingBatchTx(requests []*FundingRequest, client spamoortypes.Client, batcher *TxBatcher) (*types.Transaction, error) {
	if client == nil {
		client = pool.clientPool.GetClient(spamoortypes.SelectClientByIndex, 0, "")
		if client == nil {
			return nil, fmt.Errorf("no client available")
...
func (pool *WalletPool) buildWalletReclaimTx(ctx context.Context, childWallet spamoortypes.Wallet, client spamoortypes.Client, target common.Address) (*types.Transaction, error) {
	if client == nil {
		client = pool.clientPool.GetClient(spamoortypes.SelectClientByIndex, 0, "")
		if client == nil {
			return nil, fmt.Errorf("no client available")
...
func (pool *WalletPool) CollectPoolWallets(walletMap map[common.Address]spamoortypes.Wallet) {
...
func (pool *WalletPool) CheckChildWalletBalance(childWallet spamoortypes.Wallet) error {
	client := pool.clientPool.GetClient(spamoortypes.SelectClientRandom, 0, "")
	if client == nil {
		return fmt.Errorf("no client available")
	}
...
func (pool *WalletPool) ReclaimFunds(ctx context.Context, client spamoortypes.Client) error {
	...
	reclaimWallet := func(wallet spamoortypes.Wallet) {
		reclaimWg.Add(1)
		reclaimChan <- struct{}{}
	...
	if len(reclaimTxs) > 0 {
		...
		for _, tx := range reclaimTxs {
			go func(tx *reclaimTx) {
				...
				err := pool.txpool.SendTransaction(ctx, tx.wallet, tx.tx, &spamoortypes.SendTransactionOptions{
					Client: client,

...
func (s *Scenario) sendTx(ctx context.Context, txIdx uint64) (scenario.ReceiptChan, *types.Transaction, *spamoor.Client, *spamoor.Wallet, error) {
	...
	baseFeeWei, tipFeeWei := spamoor.ResolveFees(s.options.BaseFee, s.options.TipFee, "", "")
	...
	// Yaegi workaround: create channel with explicit underlying type and cast
	rawChan := make(chan *types.Receipt, 1)
	receiptChan := scenario.ReceiptChan(rawChan)

	// Create callback function that uses the raw channel to help Yaegi's type analysis

...
Child wallets are derived from the root wallet using a simple key derivation based on hashing the parent key with a scenario-specific seed. Each scenario gets its own unique pool of child wallets that are automatically funded and managed.

**Important**: The seed must be unique per scenario to avoid wallet address conflicts between different scenarios. Spamoor uses the seed to generate deterministic child wallet private keys by hashing the root private key with the seed and wallet index.
